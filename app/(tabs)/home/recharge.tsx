...
export default function RechargeScreen() {
  ...
  const BLE_READ_ERRORS: Record<string, { title: string; message: string; type: "error" | "warning" }> = {
    NO_DEVICE: { title: "Not Connected", message: "No device connected. Please go back and reconnect.", type: "error" },
    BLE_ERROR: { title: "Communication Error", message: "BLE communication failed. Move card closer and try again.", type: "error" },
    NO_CARD: { title: "No Card Detected", message: "Place your RFID card flat on the reader and try again.", type: "warning" },
    NO_CARD_DETECTED: { title: "No Card Detected", message: "Place your RFID card flat on the reader and try again.", type: "warning" },
    COMMUNICATION_FAILED: { title: "Device Timeout", message: "Device did not respond. Please try again.", type: "error" },
    UNKNOWN: { title: "Unknown Error", message: "Something went wrong. Please try again.", type: "error" },
  };
  ...
  const BLE_RECHARGE_ERRORS: Record<string, { title: string; message: string; type: "error" | "warning" }> = {
    ...
    BLE_ERROR: { title: "Communication Error", message: "BLE communication failed. Move card closer and try again.", type: "error" },
    NO_CARD: { title: "No Card Detected", message: "Place your RFID card flat on the reader and try again.", type: "warning" },
    NOT_INIT: { title: "Card Not Initialized", message: "This card has not been initialized yet. Please initialize it first.", type: "warning" },
    COMMUNICATION_FAILED: { title: "Device Timeout", message: "Device did not respond. Please try again.", type: "error" },
    FAIL: { title: "Card Update Failed", message: "Card update failed. Please try again.", type: "error" },
    UNKNOWN: { title: "Unknown Error", message: "Something went wrong. Please try again.", type: "error" },
  };

  const parseServerError = (rawError: string): { title: string; message: string; type: "error" | "warning" } => {
    let msg = "";
    try {
    ...
    if (msg.includes("inactive") || msg.includes("deactivated") || msg.includes("blocked"))
      return { title: "Card Inactive", message: "This card has been deactivated. Please contact admin.", type: "warning" };
    if (msg.includes("not found") || msg.includes("not registered"))
      return { title: "Card Not Registered", message: "This card is not registered. Please initialize it first.", type: "warning" };
    if (msg.includes("machine not found") || msg.includes("balance unavailable") || msg.includes("insufficient") || msg.includes("machine balance"))
      return { title: "Machine Balance Low", message: "Machine does not have enough balance. Please contact admin.", type: "warning" };
    if (msg.includes("failed to recharge"))
      return { title: "Recharge Failed", message: "Server could not process this recharge. Please try again.", type: "error" };
    return { title: "Server Error", message: "This card is deactivated.", type: "error" };
  };
  ...
  const handleRecharge = async () => {
    ...
    readCardIdBLE(async (readResult) => {
      ...
      rechargeCardBLE(String(machineId), cleanAmount, (bleResult) => {
        ...
        setAlertMessage(`₹${finalAmount} added to your card.\nNew Balance: ₹${bleResult.balance ?? "—"}`);
        setShowAlert(true);
      });
    });
  };
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.container}>
        ...
        <View style={styles.amountSection}>
          ...
          <Text style={styles.amountHint}>Tap your card on the reader when prompted</Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  header: {
    ...
    justifyContent: "space-between", marginBottom: 28, // ← more breathing room below header
  },
  backBtn: {
  ...
  amountLabel: { fontSize: 13, color: "rgba(255,255,255,0.4)", letterSpacing: 2, fontWeight: "600", marginBottom: 16 },
  ...
  amountUnderline: { width: 120, height: 2, borderRadius: 1, backgroundColor: "rgba(242,203,7,0.3)", marginTop: 8, marginBottom: 14 },
  amountHint: { fontSize: 12, color: "rgba(255,255,255,0.3)", textAlign: "center", paddingHorizontal: 40 },

  quickRow: { flexDirection: "row", justifyContent: "center", gap: 10, marginBottom: 32 },
  quickChip: { paddingHorizontal: 16, paddingVertical: 8, borderRadius: 20, borderWidth: 1, borderColor: "rgba(242,203,7,0.2)", backgroundColor: "rgba(242,203,7,0.06)" },
  ...
  submitArrow: { width: 32, height: 32, borderRadius: 16, backgroundColor: "rgba(26,14,79,0.2)", alignItems: "center", justifyContent: "center" },
});